...
const FloatingCard = forwardRef(function FloatingCard(
  { card, onPointerDown, width, height, dimmed, hidden },
  ref
) {
  const cat = CATEGORIES[card.category];
  const cardW = width || card.width;
  const cardH = height || card.height;
  ...
  if (card.isPhoto) {
    return (
      ...
      >
        <div
          className="fc-photo-placeholder"
  ...
  return (
    ...
    >
      <div className="fc-accent" style={{ backgroundColor: cat.color }} />
      <div className="fc-content">
        <span className="fc-category" style={{ color: cat.color }}>
          {cat.label}
        </span>
